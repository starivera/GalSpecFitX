...
class SegmentCombiner:
    ...
    Each segment is represented as a tuple of three arrays: wavelengths, fluxes, and errors.
    """
    ...
    def __init__(self, segments: List[Tuple[np.ndarray, np.ndarray, np.ndarray]]):
        ...
        :param segments: List of tuples, where each tuple contains:
                         - wavelengths: np.ndarray of floats (wavelength values, must be in ascending order).
                         - fluxes: np.ndarray of floats (corresponding flux values).
                         - errors: np.ndarray of floats (corresponding errors).
        """
        self.segments = segments

    def combine_segments(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
        Combine processed segments into a single spectrum, ensuring that the wavelengths,
        fluxes, and errors are sorted by wavelength.

        :return: Tuple containing:
                 - combined_waves: np.ndarray of floats (sorted wavelengths).
                 - combined_fluxes: np.ndarray of floats (corresponding sorted fluxes).
                 - combined_errors: np.ndarray of floats (corresponding sorted errors).
        """
...
class LibraryHandler(ABC):
    ...
    @abstractmethod
    def retrieve_templates(self, velscale: float, age_range: List[float], metal_range: List[float], FWHM_gal: float) -> any:
        """
...
class StarburstLibraryHandler(LibraryHandler):
    ...
    def retrieve_templates(self, velscale: float, age_range: List[float], metal_range: List[float], FWHM_gal: float) -> any:
        ...
        starburst99_lib = self.lib.starburst(pathname, velscale, self.lib_path, self.evol_track, age_range=age_range, metal_range=metal_range, FWHM_gal=FWHM_gal)
...
class BPASSLibraryHandler(LibraryHandler):
    ...
    def __init__(self, IMF_slope: str, star_form: str, lib_path: str, norm_range: List[float]):
        """
    ...
    def retrieve_templates(self, velscale: float, age_range: List[float], metal_range: List[float], FWHM_gal: float) -> any:
        ...
        bpass_lib = self.lib.bpass(pathname, velscale, self.lib_path, age_range=age_range, metal_range=metal_range, norm_range=self.norm_range, FWHM_gal=FWHM_gal)
...
class TemplateRetrieval:
    ...
    def retrieve_spectral_templates(self, age_range: List[float], metal_range: List[float], default_noise: float, FWHM_gal: float) -> Tuple[any, float]:
        """
        ...
        library = self.library_handler.retrieve_templates(velscale, age_range, metal_range, FWHM_gal)
        ...
        if n_iterations is not None:
            ...
            for _ in np.arange(n_iterations):
                ...
                try:
                    ...
                    mean_age, mean_z = library.plot(light_weights, plot=False)
...
class SpectrumProcessor:
    ...
    This class handles spectral data processing, including masking spectral lines, combining spectra, retrieving templates, and fitting spectra with pPXF.
    """
    ...
    def __init__(self, config: dict, segments: List[Tuple[np.ndarray, np.ndarray, np.ndarray]]):
        ...
        :param config: Configuration dictionary with keys controlling processing options, such as:
                       - 'segments': Spectral data segments.
                       - 'absorp_lam': List of absorption line wavelengths.
                       - 'output_path': Path to save output files.
        :param segments: List of tuples, where each tuple contains three arrays (wavelength, flux, error).
        """
        self.config = config
        self.segments = segments
    ...
    def mask_spectral_lines(self, R: float, n_pix: Tuple[int, int]) -> List[Tuple[float, float]]:
        ...
        :param n_pix: A tuple (n_left, n_right) specifying the pixel range for masking around rest wavelengths.
    ...
    def process_combined_spectrum(self, library_handler: LibraryHandler) -> None:
        """
        Process the spectral data through steps like combining spectra, retrieving templates,
        generating emission lines, stacking templates, and fitting the spectrum with pPXF.
        ...
        library, velscale = template_retrieval.retrieve_spectral_templates(age_range = self.config['age_range'], metal_range = self.config['metal_range'], default_noise = self.config['default_noise'], FWHM_gal = self.config['FWHM_gal'])
        ...
        fit_kwargs = {
            ...
            if key not in ['age_range', 'metal_range', 'start_stars', 'start_gas', 'FWHM_gal', 'output_path', 'absorp_lam', 'segments', 'default_noise', 'dust_stars', 'dust_gas', 'bounds_stars', 'bounds_gas', 'fixed_stars', 'fixed_gas', 'n_iterations'
            ]
        }
