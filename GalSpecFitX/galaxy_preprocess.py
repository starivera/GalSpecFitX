...
class GalaxySpectrum:
    """Manage a galaxy spectrum: read, process, and visualize."""
    ...
    def __init__(self, filename: str, segment: str):
        ...
        segment : str
            HDU name containing the spectrum data.
        """
        if not isinstance(segment, str):
            raise TypeError("segment must be an HDU name (string), not an integer.")
        self.filename = filename
        self.segment = segment

    def get_data(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
        Retrieve spectrum data from the FITS file using the HDU name.
        ...
        with fits.open(self.filename) as hdul:
            # Case-insensitive match for HDU name
            hdu = next((h for h in hdul if h.name.upper() == self.segment.upper()), None)
            if hdu is None:
                available = [h.name for h in hdul]
                raise KeyError(
                    f"HDU named '{self.segment}' not found in {self.filename}. "
                    f"Available HDUs: {available}"
                )

            if hdu.data is None:
                raise ValueError(f"HDU '{self.segment}' has no data.")

            data = hdu.data

        required_cols = ("wavelength", "flux", "error")
        if not hasattr(data, "names") or not all(c in data.names for c in required_cols):
            raise KeyError(
                f"HDU '{self.segment}' is missing one or more required columns: {required_cols}"
            )

        return data["wavelength"], data["flux"], data["error"]

    def create_new_table(
        self, spectrum: Tuple[np.ndarray, np.ndarray, np.ndarray], output_path: str, config_filename: str,
    ) -> str:
        """
        Save a new FITS table containing preprocessed spectrum data.

        Parameters
        ----------
        spectrum : tuple of np.ndarray
            (wavelengths, fluxes, errors)
        output_path : str
            Directory where the FITS file will be saved.
        ...
        # Define output file
        file_name = os.path.join(output_path, f"bestfit_{config_filename}.fits")

        # Create HDU and write
        new_hdu = fits.BinTableHDU(new_table, name=f"PREPROCESSED_SPECTRUM")

        primary_hdu = fits.PrimaryHDU()
        primary_hdu.header["SEGMENT"] = self.segment  # Store as metadata

        fits.HDUList([primary_hdu, new_hdu]).writeto(file_name, overwrite=True)

        return file_name

    def create_plots(
        self, spectrum: Tuple[np.ndarray, np.ndarray, np.ndarray], output_path: str, config_filename: str, show: bool = False
    ) -> None:
        """
        Plot preprocessed spectrum using Matplotlib and save an interactive Plotly version.
        ...
        spectrum : tuple of np.ndarray
            (wavelengths, fluxes, errors)
        output_path : str
            Directory where the HTML plot will be saved.
        show : bool, optional
            Whether to display the Matplotlib plot interactively (default: False).
        """
        os.makedirs(output_path, exist_ok=True)
        wavelengths, fluxes, errors = spectrum

        fig, ax = plt.subplots(figsize=(11, 5))
        ax.plot(wavelengths, fluxes, label="Spectrum")
        ax.plot(wavelengths, errors, linestyle='None', marker='.', markersize=1, label='Error')
        ax.set_title(f"{self.segment}: Dereddened, Deredshifted, and Rebinned Spectrum")
        ...
        # Save Plotly HTML file
        html_path = os.path.join(output_path, f"{config_filename}_preprocessed.html")
